...
export default function MonOut() {
    ...
    useEffect(() => {
        async function fetchData() {
            const resultMoney = await dispatch(fetchMoney('sub')).unwrap();
        }
        fetchData();
    }, [load])
    ...
    const handleSubMoney = async () => {
        if (typeof parseInt(value) === 'number' && parseInt(value) > 0 && value !== '') {
            const result = await dispatch(postMoney({ type: 'sub', amount: parseInt(value) })).unwrap();
            // setValue('');
            setShowNumPad(!showNumPad);
            setLoad(!load);
            const toastMessage = result.data.message;
            const toastFunction = result.data.success ? toast.success : toast.error;
    ...
    return (
        <div className="w-full h-screen flex flex-row relative overflow-hidden">
            ...
            <Box style={showNumPad ? styles.open : styles.closed} className='bg-[#EFEFEF] w-full h-[40%] absolute bottom-0 left-0 flex flex-col items-center'>
                <Box sx={{ width: '70%', height: '10%', display: 'flex', marginTop: 2 }}>
                    <TextField
                        type="text"
                        variant='outlined'
                        color='secondary'
